 ...
 * The CopiedConnectionPool uses a {@link PoolProperties} object for storing all the meta information about the connection pool.
...
public class CopiedConnectionPool {
    /**
     ...
     * Logger
     */
    private static final Log log = LogFactory.getLog(CopiedConnectionPool.class);
     ...
     * @throws SQLException
     */
    public CopiedConnectionPool(PoolConfiguration prop) throws SQLException {
    ...
    public Constructor<?> getProxyConstructor(boolean xa) throws NoSuchMethodException {
        ...
        if (proxyClassConstructor == null ) {
            Class<?> proxyClass = xa ?
                    Proxy.getProxyClass(org.apache.tomcat.jdbc.pool.CopiedConnectionPool.class.getClassLoader(), new Class[] {java.sql.Connection.class,javax.sql.PooledConnection.class, javax.sql.XAConnection.class}) :
                    Proxy.getProxyClass(org.apache.tomcat.jdbc.pool.CopiedConnectionPool.class.getClassLoader(), new Class[] {java.sql.Connection.class,javax.sql.PooledConnection.class});
...
//===============================================================================
    ...
    protected class ConnectionFuture implements Future<Connection>, Runnable {
        ...
        public ConnectionFuture(PooledConnection pc) throws SQLException {
            this.pc = pc;
            result = org.apache.tomcat.jdbc.pool.CopiedConnectionPool.this.setupConnection(pc);
            configured.set(true);
        }
        /**
         * {@inheritDoc}
        ...
        public boolean cancel(boolean mayInterruptIfRunning) {
            if (pc!=null) {
                ...
                org.apache.tomcat.jdbc.pool.CopiedConnectionPool.this.cancellator.execute(this);
            }
            return true;
        }
        ...
        public void run() {
            try {
                ...
                org.apache.tomcat.jdbc.pool.CopiedConnectionPool.log.error("Unable to cancel ConnectionFuture.",x);
            }
        }
    ...
    protected class PoolCleaner extends Thread {
        protected org.apache.tomcat.jdbc.pool.CopiedConnectionPool pool;
        protected long sleepTime;
        protected volatile boolean run = true;
        PoolCleaner(String name, org.apache.tomcat.jdbc.pool.CopiedConnectionPool pool, long sleepTime) {
            super(name);
            this.setDaemon(true);
            this.pool = pool;
