...
*/
    ...
    static int find(LinkedListNode list, LinkedListNode sublist) {
        ...
        while (list!=null){
            if(list.val.equals(sublist.val)){
                if(checkOthers(list.next,sublist.next)){
                    return index;
                }else {
                    index++;
                    list=list.next;
    ...
    private static boolean checkOthers(LinkedListNode mainList, LinkedListNode subList) {
        if(mainList!=null && subList==null){
            return true;
        }else if((mainList!=null && subList!=null)){
            if(mainList.val.equals(subList.val)){
                return checkOthers(mainList.next,subList.next);
            }
        }else if(mainList==null && subList==null){
            return true;
        }
